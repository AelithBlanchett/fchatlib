...
class FChatLib {
    ...
    throwError(args, errorDescription){
        parent.sendMessage("Error: Please message "+parent.config.master+" with the following content:\n Error at "+new Date().toLocaleString()+" on command "+JSON.stringify(args)+" in channel "+args.channel+" with error: "+errorDescription.toString(), args.channel);
    }
