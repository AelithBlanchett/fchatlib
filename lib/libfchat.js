...
class FChatLib {
    ...
    constructor(options){
        ...
        if(typeof options !== 'object'){
            console.log('Wrong parameters passed. The username, password, and character fields are required. Example: {username: "MyAcc", password: "secret", character: "MyNewBot"}');
            process.exit();
        }
        else{
            this.config = options;
            if(this.config.username == undefined || this.config.username == "" || this.config.password == undefined || this.config.password == "" || this.config.character == undefined || this.config.character == ""){
                console.log('Wrong parameters passed. The username, password, and character fields are required. Example: {username: "MyAcc", password: "secret", character: "MyNewBot"}');
                process.exit();
            }
        }
        ...
        this.channels = {};
        try {
            if (fs.statSync(process.cwd() + "/config/config.rooms.js")) {
                this.channels = jsonfile.readFileSync(process.cwd() + "/config/config.rooms.js");
            }
        }
        catch(err){}

        if(this.config.room !== undefined && Object.keys(this.channels)[this.config.room] == undefined){
    ...
    isUserChatOP(username, channel){
        if(parent.getChatOPList(channel).indexOf(username) != -1){
            return true;
        }
        return false;
    }
